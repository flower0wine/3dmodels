...
export default function ObjModelLoader({
  ...
  const retryLoading = useCallback(() => {
    ...
    // 设置重试状态
    setRetrying(true);
    console.log(`OBJ模型加载失败，将在${retryDelay}ms后进行第${retryCount + 1}次重试`);
      ...
      if (isMounted.current && !errorReported.current && !retrying) {
        ...
        if (retryCount < maxRetries) {
          ...
          if (onError) {
            ...
            requestAnimationFrame(() => {
              if (isMounted.current) {
                onError(`加载OBJ模型失败(重试${maxRetries}次后): ${e?.toString ? e.toString() : '未知错误'}`);
              }
            });
          }
        }
    ...
    if (isMounted.current && !errorReported.current && !retrying) {
      ...
      if (retryCount < maxRetries) {
        ...
        if (onError) {
          ...
          requestAnimationFrame(() => {
            if (isMounted.current) {
              onError(`加载OBJ模型失败(重试${maxRetries}次后): ${e?.message || '未知错误'}`);
            }
          });
        }
      }
